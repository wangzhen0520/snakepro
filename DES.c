...
const char BEGIN = 0;
const char END = 1;
void showProcess(char flag, long size, long total);
...
int DES_Encrypt(char *plainFile, char *keyStr,char *cipherFile){
	...
	while(!feof(plain)){
		...
		if((count = fread(plainBlock,sizeof(char),8,plain)) == 8){
            ...
            encSize += count;
			showProcess(BEGIN, encSize, fileLen);
		}
	}
	if(count){
		//填充
		...
		fwrite(cipherBlock,sizeof(char),8,cipher);
        encSize += count;
        showProcess(END, encSize, fileLen);
	}
	fclose(plain);
	fclose(cipher);
	return OK;
...
int DES_Decrypt(char *cipherFile, char *keyStr,char *plainFile){
	...
	while(1){
		...
		if(times < fileLen){
			...
			decSize += 8;
			showProcess(BEGIN, decSize, fileLen);
		}
		else{
			break;
		}
	...
	decSize += 8;
	showProcess(END, decSize, fileLen);
...
void showProcess(char flag, long size, long total)
{
	...
	int progress = (int)(size * 1.0 / total * 100);
	if (lastProgress != progress)
	{
		ret = display_progress(progress, ret);
